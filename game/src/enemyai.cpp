...
enemyai::enemyai() : m_timer(0.0f), m_speed(.6f) {}
enemyai::~enemyai() {}

void enemyai::initialise(engine::ref<engine::game_object> object, glm::vec3 position)
{
	m_object = object;
	m_object->set_forward(glm::vec3(0.f, 0.f, 1.f));
	m_object->set_position(glm::vec3(-3.f, 0.5, -2.f));
	m_object->animated_mesh()->set_default_animation(1);
}

void enemyai::on_update(const engine::timestep& time_step, glm::vec3 position, bool hard, bool started)
{
	...
	//} else   if (hard)
	//{
	//	glm::vec3 forward_towards_player = { (position.x - m_object->position().x) * 5.f, 0.f, (position.z - m_object->position().z) * 5.f };
	//}
	//else
	//{
	//	glm::vec3 forward_towards_player = { (position.x - m_object->position().x) , 0.f, (position.z - m_object->position().z) };
	...
	// To be implemented: use case switch and random number from a range 0-2 to assign an AI for a duration:

	////change to an event rather than key
	//if (engine::input::key_pressed(engine::key_codes::KEY_SPACE)) 
	//	jump();
		
	//// AI 1 - waiting 
	//if (forward_towards_player.z < .5f)
	//{
	//	idle();
	//}
