...
void engine::perspective_camera::on_update(const timestep& timestep)
{
	...
	if (input::key_pressed(engine::key_codes::KEY_ENTER) || input::key_pressed(engine::key_codes::KEY_H)) // Press Enter to start the game 
		is_game_started = true;

	if (is_game_started)
	{
		float y_value = m_position.y;	// fix the camera to the XZ plane as this is a first person game
		...
		m_position.y = y_value;	// fix the camera to the XZ plane

		// ----------------------- crouch ---------------------------------------
		if (input::key_pressed(engine::key_codes::KEY_C) && !is_crouched)
		{
			standing_up_position = m_position.y;
			m_position.y = 0.7f * m_position.y;
			is_crouched = true;
			set_movement_speed(.85f);

		}
		else if (input::key_released(engine::key_codes::KEY_C) && is_crouched)
		{
			m_position.y = standing_up_position;
			is_crouched = false;
